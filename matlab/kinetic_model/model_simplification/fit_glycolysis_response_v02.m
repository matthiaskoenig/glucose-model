 ...
 for k=1:9
    ...
    switch k
        case 1
            % panel 1 - HGP and HGU
            p1 = surf(x,y,z1');
            title('v_{HGU} (detailed)', 'FontWeight', 'bold', 'FontSize', 14)
        case 2
            % panel 2 - glycolysis and gluconeogenesis
            p1 = surf(x,y,z2');
            title('v_{GLY} (detailed)', 'FontWeight', 'bold')
        case 3
            % panel 3 - glycogenolyis and glycogen synthesis
            p1 = surf(x,y,z3');
            title('v_{GS} (detailed)', 'FontWeight', 'bold')
            zlim([-30 30])
            
        case 4
            p1 = surf(x,y,fit_z1');
            title('v_{HGU} (reduced)', 'FontWeight', 'bold')
        case 5
            p1 = surf(x,y,fit_z2');
            title('v_{GLY} (reduced)', 'FontWeight', 'bold')

        case 6
            p1 = surf(x,y,fit_z3');
            title('v_{GS} (reduced)', 'FontWeight', 'bold')
            zlim([-30 30])
        
        zlim_lim = 20;
        case 7
            p1 = surf(x,y, abs(z1'-fit_z1'));
            title('\Delta v_{HGU}', 'FontWeight', 'bold')
            zlim([0, zlim_lim])
        case 8
            p1 = surf(x,y,abs(z2'-fit_z2'));
            title('\Delta v_{GLY}', 'FontWeight', 'bold')
            zlim([0, zlim_lim])
        case 9
            p1 = surf(x,y,abs(z3'-fit_z3'));
            title('\Delta v_{GS}', 'FontWeight', 'bold')       
            zlim([0, zlim_lim])
