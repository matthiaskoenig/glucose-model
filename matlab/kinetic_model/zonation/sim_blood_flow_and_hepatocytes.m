function [] = sim_blood_flow_and_hepatocytes()
%%SIMPLE_ZONATTION Simulation of blood flow model with adjacent
% hepatocytes.
% Generates the outer blood compartments and the blood flow model from pp
% to pv hepatocytes. Simulates blood flow model and the hepatocytes in one
% common ode system.
%   
...
% Combine all the information into one big ODE system consisting of the
% blood metabolites and the internal metabolites of the ode system.
% here the complex stuff is done

[t,x] = ode15s(@(t,x) sim_blood_flow_and_hepatocytes_ode(t, x, pars), tspan, x0, odeset('RelTol', 1e-6));


% Set the constant values of the simulation
% This are the periportal metabolite concentrations as well as the internal
% metabolites which are kept constant.
% [TODO] better set the derivatives zero and not manually set the constant
% concentrations.
for k=1:length(pars.names)
    f = pars.f_tc{k};
