function [V, TD] = model_glycolysis(t, S, mpars)
...
%% glucose time course glycogen synthesis
%{
glc_min = 5.5;
glc_max = 8.0;
glc_ext = glc_min + (glc_max-glc_min)/300 * t;
%}
%% glucose time course glycogenolysis
%glc_min = 3.6;
%glc_max = 6.0;
%glc_ext = glc_max - (glc_max-glc_min)/3900 * t;

%% glucose time course hgp
%{
glc_min = 3.6;
glc_max = 4.4;
if t <= 600
...
elseif t > 3900
    ...
    glc_ext = glc_max - (glc_max-glc_min)/3300 * (t-600);
end
%}
...
% Glucose timecourse
if isfield(mpars, 'f_glc_ext')
   disp('use external glucose concentration')
   glc_ext = mpars.f_glc_ext(t); 
end


%% cherrington1976 simulation
% Main setting for the simulations. For the glucose time course simulations
% the plasma glucose time courses have to be loaded and used
cherrington1976 = false;
%cherrington1976 = mpars.cherrington1976;
...
if cherrington1976
    data = mpars.data;
    % get the concentration profiles for the hormones by linear
    % interpolation of the data points
    ins = data.ins;
    glu = data.glu;
    gamma = gamma_f(ins, glu);
    
    % get the glucose profiles by linear interpolation of the data points
    glc_ext = data.glc;
end
    ...
    % test glycogen effect
    %{
    if mpars.glycogen_gamma_defect == true;
        disp('Glycogen Hormone Defect !!!')
        gamma_gs = gamma_f(mpars.insulin(5.0), mpars.glucagon(5.0), mpars.epinephrine(5.0));
        gamma_gp = gamma_f(mpars.insulin(5.0), mpars.glucagon(5.0), mpars.epinephrine(5.0));
    end
    %}
...
%v = Vmax/km * (glc_ext - glc/keq) /(1 + glc_ext/km + glc/km);
...
v5 = scale_glyglc * Vmax(5)/v5_km_glc1p * v5_td / (1 + glc1p/v5_km_glc1p + glc6p/v5_km_glc6p);
...
v6 = scale_glyglc * Vmax(6)/(v6_km_utp*v6_km_glc1p) * v6_td / ( (1 + utp/v6_km_utp)*(1 + glc1p/v6_km_glc1p) + (1 + udpglc/v6_km_udpglc)*(1 + pp/v6_km_pp) - 1 );
...
v7 = scale_glyglc * Vmax(7) * pp/(pp + v7_km_pp);
...
v8_native = scale_glyglc * Vmax(8) * v8_storage_factor * udpglc / (v8_k_udpglc_native + udpglc);
v8_phospho = scale_glyglc * Vmax(8) * v8_storage_factor * udpglc / (v8_k_udpglc_phospho + udpglc); 
...
v9_vmax_native = scale_glyglc * Vmax(9) * v9_fmax * (v9_base_amp_native + (v9_max_amp_native - v9_base_amp_native) *amp/(amp+v9_ka_amp_native));
...
v9_vmax_phospho = scale_glyglc * Vmax(9) * v9_fmax * exp(-log(2)/v9_ki_glc_phospho * glc);
...
v11 = scale_glyglc * Vmax(11) / (v11_km_atp * v11_km_udp) * v11_td / ( (1 + atp/v11_km_atp)*(1 + udp/v11_km_udp) + (1 + adp/v11_km_adp)*(1 + utp/v11_km_utp) - 1) ;
