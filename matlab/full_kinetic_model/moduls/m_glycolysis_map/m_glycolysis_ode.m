function [v] = m_glycolysis_ode(x, t, mpars)
...
utp         = x(mpars.x_ind('utp'));
udp         = x(mpars.x_ind('udp'));
gtp         = x(mpars.x_ind('gtp'));
...
glc1p       = x(mpars.x_ind('glc1p'));
udpglc      = x(mpars.x_ind('udpglc'));
glyglc      = x(mpars.x_ind('glyglc'));
...
bpg13       = x(mpars.x_ind('bpg13'));
pg3         = x(mpars.x_ind('pg3'));
pg2         = x(mpars.x_ind('pg2'));
pep         = x(mpars.x_ind('pep'));
pyr         = x(mpars.x_ind('pyr'));
oaa         = x(mpars.x_ind('oaa'));
lac         = x(mpars.x_ind('lac'));
glc_ext     = x(mpars.x_ind('glc_{ext}'));
lac_ext     = x(mpars.x_ind('lac_{ext}'));
co2_mito    = x(mpars.x_ind('co2_{mito}'));
p_mito      = x(mpars.x_ind('p_{mito}'));
oaa_mito    = x(mpars.x_ind('oaa_{mito}'));
pep_mito    = x(mpars.x_ind('pep_{mito}'));
...
v1_km = 42;                                 % [mM]
v(mpars.v_ind('GLUT2')) = Vmax('GLUT2')/v1_km * v1_td /(1 + glc_ext/v1_km + glc/v1_km);
...
v(mpars.v_ind('GK')) = Vmax('GK') * v2_gc_free * atp/(v2_km_atp + atp) * glc^v2_n/(glc^v2_n + v2_km_glc^v2_n);
...
v3_km_glc6p = 2;            % [mM]
v(mpars.v_ind('G6PASE')) = Vmax('G6PASE') * glc6p / (v3_km_glc6p + glc6p);
...
v(mpars.v_ind('GPI')) = Vmax('GPI')/v4_km_glc6p * v4_td / (1 + glc6p/v4_km_glc6p + fru6p/v4_km_fru6p);
...
v(mpars.v_ind('G16PI')) = Vmax('G16PI')/v5_km_glc1p * v5_td / (1 + glc1p/v5_km_glc1p + glc6p/v5_km_glc6p);
...
v(mpars.v_ind('UPGASE')) = Vmax('UPGASE')/(v6_km_utp*v6_km_glc1p) * v6_td / ( (1 + utp/v6_km_utp)*(1 + glc1p/v6_km_glc1p) + (1 + udpglc/v6_km_udpglc)*(1 + pp/v6_km_pp) - 1 );
...
v(mpars.v_ind('PPASE')) = Vmax('PPASE') * pp/(pp + v7_km_pp);
...
v8_native = Vmax('GS') * v8_storage_factor * udpglc / (v8_k_udpglc_native + udpglc);
v8_phospho = Vmax('GS') * v8_storage_factor * udpglc / (v8_k_udpglc_phospho + udpglc); 
v(mpars.v_ind('GS')) = (1 - gamma_gs)* v8_native + gamma_gs * v8_phospho;
...
v9_vmax_native = Vmax('GP') * v9_fmax * (v9_base_amp_native + (v9_max_amp_native - v9_base_amp_native) *amp/(amp+v9_ka_amp_native));
...
v9_vmax_phospho = Vmax('GP') * v9_fmax * exp(-log(2)/v9_ki_glc_phospho * glc);
...
v(mpars.v_ind('GP')) = (1 - gamma_gp) * v9_native + gamma_gp * v9_phospho;
...
v(mpars.v_ind('NTK (GTP)')) = Vmax('NTK (GTP)') / (v10_km_atp * v10_km_gdp) * v10_td / ( (1 + atp/v10_km_atp)*(1 + gdp/v10_km_gdp) + (1 + adp/v10_km_adp)*(1 + gtp/v10_km_gtp) - 1) ;
...
v(mpars.v_ind('NTK (UTP)')) = Vmax('NTK (UTP)') / (v11_km_atp * v11_km_udp) * v11_td / ( (1 + atp/v11_km_atp)*(1 + udp/v11_km_udp) + (1 + adp/v11_km_adp)*(1 + utp/v11_km_utp) - 1) ;
...
v(mpars.v_ind('AK')) = Vmax('AK') / (v12_km_atp * v12_km_amp) * v12_td / ( (1+atp/v12_km_atp)*(1+amp/v12_km_amp) + (1+adp/v12_km_adp)*(1+adp/v12_km_adp) - 1); 
...
v13_native = Vmax('PFK2') * power(fru6p, v13_n_native) / ...
    (power(fru6p, v13_n_native) + power(v13_k_fru6p_native, v13_n_native)) * ...
    (atp / (atp + v13_k_atp_native));
v13_phospho = Vmax('PFK2') * power(fru6p, v13_n_phospho) / ...
    (power(fru6p, v13_n_phospho) + power(v13_k_fru6p_phospho, v13_n_phospho)) * ...
    (atp / (atp + v13_k_atp_phospho));

v(mpars.v_ind('PFK2')) = (1 - gamma_pfk2) * v13_native + gamma_pfk2 * v13_phospho;
...
v14_native  = Vmax('FBP2')/(1 + fru6p/v14_ki_fru6p_native) * fru26bp / ( v14_km_fru26bp_native + fru26bp);
v14_phospho = Vmax('FBP2')/(1 + fru6p/v14_ki_fru6p_phospho) * fru26bp / ( v14_km_fru26bp_phospho + fru26bp);
v(mpars.v_ind('FBP2')) = (1-gamma_fbp2) * v14_native + gamma_fbp2 * v14_phospho;
...
v(mpars.v_ind('PFK1')) = Vmax('PFK1') * (1 - 1./(1 + fru26bp/v15_ka_fru26bp)) * ...
    fru6p*atp / (v15_ki_fru6p*v15_km_atp + v15_km_fru6p*atp + v15_km_atp*fru6p + atp*fru6p);
...
v(mpars.v_ind('FBP1')) = Vmax('FBP1') / (1 + fru26bp/v16_ki_fru26bp) * fru16bp/(fru16bp + v16_km_fru16bp);
...
v(mpars.v_ind('ALD')) = Vmax('ALD')/v17_km_fru16bp * v17_td/(1 + fru16bp/v17_km_fru16bp + grap/v17_ki1_grap + dhap*(grap + v17_km_grap)/(v17_km_dhap*v17_ki1_grap) + (fru16bp*grap)/(v17_km_fru16bp*v17_ki2_grap));
...
v18_km_dhap = 0.59;             %[mM]
v18_km_grap = 0.42;             %[mM]
v(mpars.v_ind('TPI')) = Vmax('TPI')/v18_km_dhap * v18_td / (1 + dhap/v18_km_dhap + grap/v18_km_grap);
...
v(mpars.v_ind('GAPDH')) = Vmax('GAPDH') / (v19_k_nad*v19_k_grap*v19_k_p) * v19_td / ( (1 + nad/v19_k_nad) * (1+grap/v19_k_grap) * (1 + p/v19_k_p) + (1+nadh/v19_k_nadh)*(1+bpg13/v19_k_bpg13) - 1);
...
v(mpars.v_ind('PGK')) = Vmax('PGK') / (v20_k_adp*v20_k_bpg13) * v20_td / ((1 + adp/v20_k_adp)*(1+bpg13/v20_k_bpg13) + (1+atp/v20_k_atp)*(1+pg3/v20_k_pg3) - 1); 
...
v21_k_pg3 = 5;      % [mM]
v21_k_pg2 = 1;      % [mM]
v(mpars.v_ind('PGM')) = Vmax('PGM') * v21_td / (pg3 + v21_k_pg3 *(1 + pg2/v21_k_pg2));
...
v(mpars.v_ind('EN')) = Vmax('EN') * v22_td / (pg2 + v22_k_pg2 *(1 + pep/v22_k_pep));
...
v23_native =  Vmax('PK') * v23_alpha_inp * pep^v23_n/(v23_pep_inp.^v23_n + pep^v23_n) * adp/(adp + v23_k_adp) * ( v23_base_act + (1-v23_base_act) *v23_f );
v23_phospho = Vmax('PK') * v23_alpha_p_inp * pep^v23_n_p/(v23_pep_p_inp.^v23_n_p + pep^v23_n_p) * adp/(adp + v23_k_adp) * ( v23_base_act_p + (1-v23_base_act_p) * v23_f_p );

v(mpars.v_ind('PK')) = (1-gamma_pk)* v23_native + gamma_pk * v23_phospho;
...
v(mpars.v_ind('PEPCK')) = Vmax('PEPCK') / (v24_k_oaa * v24_k_gtp) * v24_td / ( (1+oaa/v24_k_oaa)*(1+gtp/v24_k_gtp) + (1+pep/v24_k_pep)*(1+gdp/v24_k_gdp)*(1+co2/v24_k_co2) - 1 );

% *********************************** %
% v25 : PEPCK mito
% *********************************** %
v25_td = ( oaa_mito * gtp_mito - pep_mito*gdp_mito*co2_mito /v24_keq); 
v(mpars.v_ind('PEPCK_{mito}')) = Vmax('PEPCK_{mito}') / (v24_k_oaa * v24_k_gtp) * v25_td / ( (1+oaa_mito/v24_k_oaa)*(1+gtp_mito/v24_k_gtp) + (1+pep_mito/v24_k_pep)*(1+gdp_mito/v24_k_gdp)*(1+co2_mito/v24_k_co2) - 1 );
...
v26_k_acoa = 0.015; %[mM]
v26_n = 2.5;
v(mpars.v_ind('PC')) = Vmax('PC') * atp_mito/(v26_k_atp + atp_mito) * pyr_mito/(v26_k_pyr + pyr_mito) * co2_mito/(v26_k_co2 + co2_mito) * acoa_mito^v26_n / (acoa_mito^v26_n + v26_k_acoa^v26_n);
...
v27_k_nad = 0.984;      % [mM]
v27_k_nadh = 0.027;      % [mM]
v(mpars.v_ind('LDH')) = Vmax('LDH') / (v27_k_pyr * v27_k_nadh) * v27_td / ( (1+nadh/v27_k_nadh)*(1+pyr/v27_k_pyr) + (1+lac/v27_k_lac) * (1+nad/v27_k_nad) - 1);
...
v28_k_lac = 0.8;                % [mM]
v(mpars.v_ind('LACT')) = Vmax('LACT')/v28_k_lac * v28_td / (1 + lac_ext/v28_k_lac + lac/v28_k_lac);
...
v29_k_pyr = 0.1;                % [mM]
v(mpars.v_ind('PYRT')) = Vmax('PYRT')/v29_k_pyr * v29_td / (1 + pyr/v29_k_pyr + pyr_mito/v29_k_pyr);
...
v30_k_pep = 0.1;                % [mM]
v(mpars.v_ind('PEPT')) = Vmax('PEPT')/v30_k_pep * v30_td / (1 + pep/v30_k_pep + pep_mito/v30_k_pep);
...
v31_base = Vmax('PDH') * pyr_mito/(pyr_mito + v31_k_pyr) * nad_mito/(nad_mito + v31_k_nad*(1 + nadh_mito/v31_ki_nadh)) * coa_mito/(coa_mito + v31_k_coa*(1+acoa_mito/v31_ki_acoa));
v31_nat = v31_base * v31_alpha_nat;
v31_p   = v31_base * v31_alpha_p;
v(mpars.v_ind('PDH')) = (1 - gamma_pdh) * v31_nat + gamma_pdh * v31_p;
...
v(mpars.v_ind('CS')) = Vmax('CS')/(v32_k_oaa * v32_k_acoa) * v32_td / ... 
...
v(mpars.v_ind('NDK_{mito}')) = Vmax('NDK_{mito}') / (v33_k_atp * v33_k_gdp) * v33_td / ( (1 + atp_mito/v33_k_atp)*(1 + gdp_mito/v33_k_gdp) + (1 + adp_mito/v33_k_adp)*(1 + gtp_mito/v33_k_gtp) - 1) ;

% *********************************** %
% v34 : OAA influx
% *********************************** %
v34_td = 0;
v(mpars.v_ind('OAA_{flx}')) = Vmax('OAA_{flx}');

% *********************************** %
% v35 : Acetyl-CoA efflux
% *********************************** %
v35_td = 0;
v(mpars.v_ind('ACOA_{flx}')) = Vmax('ACOA_{flx}');

% *********************************** %
% v36 : Citrate efflux
% *********************************** %
v36_td = 0;
v(mpars.v_ind('CIT_{flx}')) = Vmax('CIT_{flx}');
