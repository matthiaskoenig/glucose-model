...
k_c16_coa_mito    = x(mpars.x_ind('k_c16_coa_{mito}'));

% ketone bodies
hmgcoa_mito    = x(mpars.x_ind('hmgcoa_{mito}'));
acac_mito       = x(mpars.x_ind('acac_{mito}'));
bhbut_mito = x(mpars.x_ind('bhbut_{mito}'));
...
% Vmax_citmal = 10;
v_citmal = Vmax('CITMAL')*(cit_mito*mal - 0.1*cit*mal_mito)./ ...
    ( (1 + cit_mito/0.13).*(1 + mal/0.78) + (1+cit/0.13).*(1+mal_mito/0.78) -1 );
...
% malcoa -> acoa
% Vmax_mcoause = 0.1;
v_mcoause = Vmax('MCOAUSE') * malcoa/(malcoa + 0.01);
...
v(mpars.v_ind('ACOA_{MITO}'))   = v_pdh - v_cs + v_c16kt3 + v_c14kt3 + v_c12kt3 + ...
                  v_c10kt3 + v_c8kt3 + v_c6kt3 + 2 * v_c4kt3 + ...
                  - v_hmgcoas + v_hmgcoal;
v(mpars.v_ind('CIT_{MITO}'))    = v_cs - v_aco - v_citmal;
v(mpars.v_ind('ISOCIT_{MITO}')) = v_aco - v_idh;
v(mpars.v_ind('AKG_{MITO}'))    = v_idh - v_kgdh - v_AAT_mito - v_Mal_a_keto;
v(mpars.v_ind('SUCCOA_{MITO}')) = v_kgdh - v_scs1 - v_scs2;
...
v(mpars.v_ind('L_C10_COA_{MITO}')) = + v_c10coacrot - v_c10hcdh;
...
v(mpars.v_ind('ACAC_{MITO}'))   = + v_hmgcoal - v_hbdh  - v_acacme ;
v(mpars.v_ind('BHBUT_{MITO}'))  = + v_hbdh - v_bhbutme;
